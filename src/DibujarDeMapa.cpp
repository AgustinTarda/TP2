...
DibujadorDeMapa::DibujadorDeMapa(BMP &mapaBase) {
	...
	RangedPixelToPixelCopy(mapaBase, 0, mapaBase.TellWidth(), 0,
			mapaBase.TellHeight(), *this->mapa, 0, 0);
}
...
void DibujadorDeMapa::dibujarPunto(unsigned int xCentro, unsigned int yCentro,
		unsigned int radio, short int rojo, short int verde, short int azul) {
	RGBApixel color;
	color.Red = rojo;
	color.Green = verde;
	color.Blue = azul;
...
void DibujadorDeMapa::dibujarPuntoIterativo(unsigned int xCentro,
		unsigned int yCentro, unsigned int radio, RGBApixel color) {
	if (radio == 0) {
...
void DibujadorDeMapa::dibujarLinea(unsigned int xInicial, unsigned int yInicial,
...
void DibujadorDeMapa::dibujarLineaIterativo(unsigned int xInicial,
		unsigned int yInicial, unsigned int xFinal, unsigned int yFinal,
		RGBApixel color) {
	if ((xInicial - xFinal) != 0 || (yInicial - yFinal) != 0) {
		...
		if (abs(desplazamientoX) <= abs(desplazamientoY)) {
			if (desplazamientoY >= 0) {
				this->dibujarLineaIterativo(xInicial, yInicial + 1, xFinal,
						yFinal, color);
			} else {
				this->dibujarLineaIterativo(xInicial, yInicial - 1, xFinal,
						yFinal, color);
			...
			if (desplazamientoX >= 0) {
				this->dibujarLineaIterativo(xInicial + 1, yInicial, xFinal,
						yFinal, color);
			} else {
				this->dibujarLineaIterativo(xInicial - 1, yInicial, xFinal,
						yFinal, color);
...
unsigned int DibujadorDeMapa::convertidorDeCoordenadasAPixelsLongitud(
		Coordenadas coordenada) {

	unsigned int pixelX;

	pixelX = int ((coordenada.getLongitud() + COORDENADA_X0)
			* DISTANCIA_PIXEL_X / DISTANCIA_COORDENADAS_X);
...
unsigned int DibujadorDeMapa::convertidorDeCoordenadasAPixelsLatitud(
		Coordenadas coordenada) {

	unsigned int pixelY;

	pixelY = int(
			(coordenada.getLatitud() + COORDENADA_Y0) * DISTANCIA_PIXEL_Y
					/ DISTANCIA_COORDENADAS_Y);
